...
# Kernel source toggle (safe default keeps existing fallback)
MYRON_USE_QCOM_KAANAPALI_KERNEL ?= false
ifeq ($(MYRON_USE_QCOM_KAANAPALI_KERNEL),true)
TARGET_KERNEL_SOURCE := kernel/qcom/kaanapali
else
...
# Persistently flashed boot.img is rejected unless it carries stock-compatible
# AVB identity. During the stock-userspace equivalence phase we intentionally
# mirror the current HyperOS boot footer props to reduce recovery handoff due
# to bootimage metadata drift. The build graph still requires a non-zero
# rollback index location for chained vbmeta generation, so we keep the
# buildable location here and only override the stock-facing props that matter
# for this compatibility phase.
BOARD_AVB_BOOT_KEY_PATH := external/avb/test/data/testkey_rsa4096.pem
BOARD_AVB_BOOT_ALGORITHM := SHA256_RSA4096
BOARD_AVB_BOOT_ROLLBACK_INDEX := 1767225600
BOARD_AVB_BOOT_ROLLBACK_INDEX_LOCATION := 2
BOARD_AVB_BOOT_ADD_HASH_FOOTER_ARGS += --prop com.android.build.boot.security_patch:2026-01-01
BOARD_AVB_BOOT_ADD_HASH_FOOTER_ARGS += --prop com.android.build.boot.fingerprint:POCO/myron_eea/myron:16/BQ2A.250705.001-BP2A.250605.031.A3/OS3.0.7.0.WPMEUXM:user/release-keys
