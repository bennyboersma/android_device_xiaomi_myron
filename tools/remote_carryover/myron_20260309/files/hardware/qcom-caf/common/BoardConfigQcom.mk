...
ifeq ($(filter $(TARGET_BOARD_PLATFORM), neo61 kaanapali),$(TARGET_BOARD_PLATFORM))
...
ifeq ($(TARGET_BOARD_PLATFORM),kaanapali)
    SOONG_CONFIG_qtidisplay_drmpp := true
    SOONG_CONFIG_qtidisplay_gralloc4 := true
    SOONG_CONFIG_qtidisplay_displayconfig_enabled := true
    SOONG_CONFIG_qtidisplay_ubwcp_headers := true
...
# Kaanapali uses the commonsys display interface headers even though it is not
# classified under the current QSSI family lists.
ifeq ($(TARGET_BOARD_PLATFORM),kaanapali)
    PRODUCT_SOONG_NAMESPACES += \
        vendor/qcom/opensource/commonsys/display \
        vendor/qcom/opensource/commonsys-intf/display
...
ifeq ($(BOARD_SUPPORTS_OPENSOURCE_STHAL),true)
    ifneq ($(filter $(LEGACY_UM_PLATFORMS),$(TARGET_BOARD_PLATFORM)),)
        ...
        ifneq ($(filter $(UM_5_10_FAMILY) $(UM_5_15_FAMILY) $(UM_6_1_FAMILY),$(TARGET_BOARD_PLATFORM)),)
            ...
            $(call soong_config_set,qtiaudio,headers_namespace,$(QCOM_SOONG_NAMESPACE)/audio/pal)
            $(call soong_config_set,qtiaudio,libarpal_namespace,$(QCOM_SOONG_NAMESPACE)/audio/pal)
        endif
    endif
endif
